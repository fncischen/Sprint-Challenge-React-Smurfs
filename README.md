...
* We can use the get/fetch methods to determine, or resolve, which data from the server we need to request.
* We can also use the post/put/delete methods, from the axios library (as "Promise" Objects) to determine which CRUD operation
we need to apply, based on which data we're requesting from the server.
* The then or catch methods allow us to apply appropriate action on the browser (i.e. change the component state data or return an error) after we've determined whether we've successfully (or not) applied the appropriate action to the server data request.
